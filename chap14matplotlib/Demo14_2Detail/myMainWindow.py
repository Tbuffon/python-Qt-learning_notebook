...
##  黑体：SimHei 宋体：SimSun 新宋体：NSimSun 仿宋：FangSong  楷体：KaiTi 
      ...
      self.__fig = None  #Figure对象
      self.__curAxes = None  #当前Axes对象
      self.__curLine = None  #当前曲线对象
      self.__createFigure()  #创建Figure和FigureCanvas对象，初始化界面
      self.__drawFig2X1()    #绘制2行1列的图表

      axesList = self.__fig.axes #图的子图列表
      for one in axesList:
         self.__comboAxes.addItem(one.get_label())  #添加到下拉列表

      legendLocs = {'best', 'upper right', 'upper left', 'lower left', 'lower right',
                    'right', 'center left', 'center right', 'lower center', 'upper center',
                     'center'}
      self.ui.combo_LegendLoc.addItems(legendLocs)  #添加到下拉列表
      styleList = mplStyle.available  #获取样式列表
      self.ui.comboFig_Style.addItems(styleList)  #添加到下拉列表


##  ==============自定义功能函数========================
   def __createFigure(self):
      self.__fig = mpl.figure.Figure()  #创建Figure对象
      figCanvas = FigureCanvas(self.__fig)  #创建FigureCanvas对象
      self.__fig.suptitle("suptoitle:matplotlib in Qt GUI",fontsize=16, fontweight='bold') #设置标题
      naviToolbar = NavigationToolbar(figCanvas, self)  #创建FigureCanvas工具栏
      actList = naviToolbar.actions()  #获取工具栏的动作列表
      count = len(actList)
      lastAction = actList[count - 1]  #最后一个动作

      labCurAxes = QLabel("当前子图")
      naviToolbar.insertWidget(lastAction, labCurAxes)  #插入一个Label组件
      self.__comboAxes = QComboBox()  #创建子图下拉列表
      self.__comboAxes.setToolTip("选择当前子图")
      self.__comboAxes.currentIndexChanged.connect(self.do_currentAxesChaned)  #当前子图改变时触发
      naviToolbar.insertWidget(lastAction, self.__comboAxes)  #插入子图下拉列表combobox
      naviToolbar.insertAction(lastAction, self.ui.actQuit)  #插入一个动作Action
      self.addToolBar(naviToolbar)  #添加作为主窗口工具栏

      splitter = QSplitter(self)  #创建分割窗口
      splitter.setOrientation(Qt.Horizontal)  #设置为水平方向
      splitter.addWidget(self.ui.toolBox)  #左侧控制面板
      splitter.addWidget(figCanvas)  #右侧FigureCanvas对象
      self.setCentralWidget(splitter)  #设置为主窗口的中央组件
   ...
   def __drawFig2X1(self):  ##初始化绘图
      ...
      y1 = np.sin(t)
      y2 = np.cos(2 * t)
      ax1.plot(t, y1, 'r-o', label="sin", linewidth=2, markersize=5)    #绘制曲线
      ax1.plot(t, y2, 'b--', label="cos", linewidth=2)
      ax1.set_xlabel('X 轴') #设置X轴标题
      ax1.set_ylabel('Y 轴') #设置Y轴标题
      ax1.set_xlim([0, 10]) #设置X轴范围
      ax1.set_title("三角函数曲线") #设置子图标题
      ax1.legend()  #显示图例
      self.__curAxes = ax1 #当前子图

      ax2 = self.__fig.add_subplot(2, 1, 2, label="magnitude plot")  #子图2
      w = np.logspace(-1, 1, 100) #频率
      mag = self.__getMag(w, zta=0.2, wn=1) #阻尼比为0.2
      ax2.semilogx(w, mag, 'g-', label=r"$\zeta=0.2$", linewidth=2) #绘制曲线
      mag = self.__getMag(w, zta=0.4, wn=1) #阻尼比为0.4
      ax2.semilogx(w, mag, 'r:', label=r"$\zeta=0.4$", linewidth=2) #绘制曲线
      mag = self.__getMag(w, zta=0.8, wn=1) #阻尼比为0.8
      ax2.semilogx(w, mag, 'b--', label=r"$\zeta=0.8$", linewidth=2) #绘制曲线
      ax2.set_xlabel('角频率(rad/sec)') #设置X轴标题
      ax2.set_ylabel('幅度(dB)') #设置Y轴标题
      ax2.set_title("二阶系统幅频曲线") #设置子图标题
      ax2.legend()  #显示图例
