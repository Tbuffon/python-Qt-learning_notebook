...
##  ==========由connectSlotsByName() 自动连接的槽函数===============        
   def on_btnCalculate_clicked(self):   ##"计算总价"按钮
      num = int(self.ui.editCount.text())  #获取数量
      price = float(self.ui.editPrice.text())  #获取单价
      total = num * price
      self.ui.editTotal.setText("%.2f" % total)  #显示总价

   @pyqtSlot(int)    ##"数量"SpinBox
   def on_spinCount_valueChanged(self,count):
      price = self.ui.spinPrice.value()  #获取单价
      self.ui.spinTotal.setValue(count * price)  #显示总价
